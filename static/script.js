...
document
  .getElementById('word_form')
  .addEventListener('submit', async function (event) {
    event.preventDefault();

    if (event.submitter.id === 'new_game') return resetBoggle();

    const word_input = document.getElementById('word');
    if (word_input.disabled || !word_input.value) return;
    ...
    const {
      data: { result, path },
    } = await axios.post('/submit', { word: word });
    switch (result) {
      case RESULT_OK:
    ...
    if (timerHandle) word_input.disabled = false;
  });
